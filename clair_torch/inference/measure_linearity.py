...
def measure_linearity(image_dataset: ImageMapDataset, device: str | torch.device,
    ...
    Args:
        image_dataset: an ImageMapDataset consisting of FrameSettings or PairedFrameSettings objects.
    ...
    """
    dataloader = DataLoader(dataset=image_dataset, batch_size=13, shuffle=False, drop_last=False,
                            pin_memory=False, collate_fn=custom_collate)

    for val_batch, std_batch, meta_batch in tqdm(dataloader, desc="Creating linearity plot"):
        ...
        valid_mask = get_pairwise_valid_pixel_mask(images, i_idx, j_idx, stds, lower=1 / 255, upper=254 / 255)
